...
def _stub_bedrock_narrative(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    the Close Note. On a workstation with AWS creds present, the lazy
    ``BedrockClient`` construction succeeds and ``generate_text`` blocks
    on the AWS API, hanging the test. The narrative is empty-safe by
    contract — the route falls back to the structured-only Close Note
