...
def test_unreconciled_internal_transfer_caps_severity_in_single_account_view() -> None:
    """Regression: in true single-account view (no transfer-IN legs at
    all in the bundle), EVERY transfer-out is unreconciled by definition
    — the detector can't distinguish "hidden account" from "owner moved
    money to a second account we just didn't see." Per the docstring,
    severity stays at 15 (medium signal for operator sanity-check)
    instead of escalating with the unmatched count.

    Prior implementation escalated to severity=40 (the cap) for any
    merchant with ≥6 outbound transfers. Demonstrated on VU Development
    2026-06: 6-8 owner-moves to CHK 7722 each month triggered max
    severity and pushed patterns_score into the fraud_score hard-decline
    band even though there was no hidden-account signal at all."""
    out_amounts = [
        Decimal("-164916.25"),
        Decimal("-161692.37"),
        Decimal("-125792.37"),
    ...
    txns = [
        _txn(
            ...
            description=f"Online Banking transfer to CHK 7722 Confirmation# {i}",
            amount=amt,
            category="transfer",
        )
        for i, amt in enumerate(out_amounts)
    ]
    pats = _by_code(_analyze(txns))
    assert "unreconciled_internal_transfer" in pats
    assert pats["unreconciled_internal_transfer"].severity == 15
    assert len(pats["unreconciled_internal_transfer"].source_ids) == 6


def test_unreconciled_internal_transfer_still_escalates_when_ins_exist() -> None:
    """When the bundle DOES contain transfer-IN legs but some outbound
    legs go unmatched, count-scaling still applies (signal is meaningful
    in that mode — we have something to fail against)."""
    txns = [
        _txn(
        ...
        # Three more unmatched OUT legs in the same bundle.
        _txn(
            posted_date=date(2026, 1, 10),
            description="TRANSFER TO XXXX9999",
    ...
    pats = _by_code(_analyze(txns))
    # 3 unmatched outs; severity = min(40, 15 + 5 * (3-1)) = 25
    assert pats["unreconciled_internal_transfer"].severity == 25
