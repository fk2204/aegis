...
> **SCOPE NOTE (added 2026-05-25) — Commera Capital is a pure ISO broker.** AEGIS is an internal pre-screening tool that the operator uses to evaluate deal quality before submitting to funder partners. AEGIS never extends financing, never generates merchant-facing disclosures, never charges merchants any fee, and never holds the regulatory primary actor role. State CFDL disclosure obligations, renewal disclosure obligations, §1071 reporting, COJ/auto-debit/forum overlay enforcement, and prescribed-form disclosure templates all sit with the funder. Phases originally designed assuming AEGIS as regulator-primary actor are marked **OBSOLETE — WRONG SCOPE** below (Phases 4, 5, 6, 8, 12). Their section text is preserved for historical reference; do not implement them.
...
**Last updated:** 2026-05-25 (scope correction; original 2026-05-17)
**Disclaimer:** This document distills published industry and legal commentary as of May 2026. It is operational guidance for engineering, not legal advice. Before activating any state's Tier 1 surface in production, a licensed attorney in that state must review the disclosure template and broker-conduct rules. See SCOPE NOTE above — the regulator-primary-actor framing is obsolete; broker-side pre-screening is in scope, funder-side compliance is not.
...
> **OBSOLETE — WRONG SCOPE (2026-05-25).** TX HB 700 auto-debit / first-priority-lien is a funder-side structuring concern, not a broker pre-screen rule. FL/GA broker advance-fee prohibitions are satisfied by Commera's operational policy of never charging merchants any fee. The `state_not_served` framing in this phase assumed AEGIS as regulator-primary actor; as a pure ISO broker, state CFDL gating belongs to the funder. Do not implement. Section preserved below for historical context.
...
> **OBSOLETE — WRONG SCOPE (2026-05-25).** Merchant-facing commercial financing disclosures are generated by the funder (the financing provider under each state's CFDL), not by Commera as broker. AEGIS does not transmit prescribed-form disclosures and does not need locked Jinja templates per state. The 11 state directories under `docs/compliance/states/{ST}/` are preserved as informational metadata but `03_disclosure_template.j2` stubs will not be populated. Do not implement. Section preserved below for historical context.
...
> **OBSOLETE — WRONG SCOPE (2026-05-25).** CA SB 362 and NY 23 NYCRR § 600.17 renewal disclosure obligations bind the financing provider (the funder), not the broker. Commera does not have a regulator-facing renewal disclosure duty. A pre-screening "is this a renewal of an existing position" classifier may still be operationally useful for deal-quality scoring (double-dipping signal), but it is not a compliance gate — separate work, not this phase. Do not implement as written. Section preserved below for historical context.
...
> **OBSOLETE — WRONG SCOPE (2026-05-25).** Confessions of judgment, auto-debit structure, and contractual forum selection are all funder-contract concerns. The funder's contract template — not AEGIS — determines whether a deal violates a state's COJ ban or forum restriction. AEGIS as a broker pre-screen does not block on these overlays; if the funder accepts the deal, the funder's contract drives compliance. Do not implement. Section preserved below for historical context.
...
> **OBSOLETE — WRONG SCOPE (2026-05-25).** CFPB §1071 small-business data collection applies to **financing providers**, not brokers — Commera as a pure ISO broker has no §1071 reporting obligation. The state CFDL watch list was premised on AEGIS gating deals by state-specific compliance status; as a broker, state-specific compliance is the funder's responsibility, so a watch list automation that drives AEGIS routing has nothing to drive. Do not implement. Section preserved below for historical context.
