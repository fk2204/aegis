...
- **Counterparty allowlist (added 2026-06-27 after shadow audit):** Transfer-OUT rows whose description contains `nys dept of labor` / `nysdol` / `irs` / `internal revenue` / `us treasury` / `state of …` / `dept of revenue` / `department of revenue` / `dept of taxation` / `unemployment insurance` / `workers comp` / `workers compensation` / `self transfer` / `own account` / `zelle to self` are excluded BEFORE the matching loop. Substring match, case-insensitive. Shadow audit showed 100% FP rate because every fire was a government/tax/payroll wire or a self-transfer — legitimate moves with no inbound counterpart on the merchant's statements. The excluded count is appended to the detail string on any remaining emit (`"N transfer(s) excluded — government/self-transfer allowlist"`).
...
- **Min-active-signals guard (added 2026-06-27 after shadow audit):** Requires ≥2 of 4 composite signals to be non-zero before firing — prevents single-signal false positives. Shadow audit showed 65% FP rate because all fires were driven by Signal 1 (math) alone or Signal 1 + 3 (math + round-cluster); Signals 2 (description uniformity) and 4 (font uniformity) were 0 on every fire. Single-signal high-score cases now exit before the composite-threshold check.
