...
def frame_monthly_revenue(
    monthly_revenue: Money, severity: SignalSeverity
) -> FactorReason:
    detail = (
    ...
    if severity == "positive":
        ...
        detail += (
            "Modest; deal economics get thin below $25K/month — verify "
            "remittance can be sustained."
        )
    elif severity == "elevated":
        detail += (
            "Genuinely thin — under $10K/month limits deal viability."
        )
    else:  # critical
        detail = (
...
def frame_nsf(
    ...
    if nsf_count == 0:
        detail = (
            f"No NSF fees observed across {statement_period_days}-day "
            "statement period."
        )
    else:
        days = max(statement_period_days, 30)
        per_mo = (Decimal(nsf_count) * Decimal("30") / Decimal(days)).quantize(
            Decimal("0.1")
        )
        detail = (
            f"{nsf_count} NSF fees over {statement_period_days} days "
            f"(~{per_mo}/month)."
        )
        if severity == "concern":
            ...
            detail += (
                " Frequent — indicates recurring distress, not isolated events."
            )
        elif severity == "critical":
            detail += (
                " High frequency — chronic NSF pattern, treat as strong distress."
            )
    return FactorReason(factor="nsf", severity=severity, detail=detail)


def frame_mca_positions(
    mca_position_count: int, severity: SignalSeverity
) -> FactorReason:
    ...
    return FactorReason(
        factor="mca_positions", severity=severity, detail=detail
    )


def frame_negative_days(
    negative_days: int, severity: SignalSeverity
) -> FactorReason:
    if negative_days == 0:
        detail = "No days with negative running balance observed."
    elif negative_days <= 3:
        detail = (
            f"{negative_days} day(s) negative across the period — "
            "occasional dip, not a pattern."
        )
        ...
        detail = (
            f"{negative_days} negative days — chronic negative cashflow."
        )
    return FactorReason(
        factor="negative_days", severity=severity, detail=detail
    )


def frame_lowest_balance(
    lowest_balance: Money, severity: SignalSeverity
) -> FactorReason:
    if severity == "positive":
        detail = (
            f"Lowest balance observed: ${lowest_balance:,.2f}. "
            "Never went negative."
        )
    else:
    ...
    return FactorReason(
        factor="lowest_balance", severity=severity, detail=detail
    )


def frame_international_concentration(
    share_pct: Decimal, severity: SignalSeverity
) -> FactorReason:
